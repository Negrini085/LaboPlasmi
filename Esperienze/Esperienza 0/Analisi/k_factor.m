...
Navg = 100; nome = 'cdisch_';
path = '/home/filippo/Desktop/CODICINI/LABO_PLASMI/Esperienze/Esperienza 0/Dati/Signals/ion_discharge/cdisch';
car_ioni = carica_segnale(path, nome, 33, 1e6, 0.001, 0.001, Navg, 0);
...
% decidiamo di lavorare considerando 100 punti per fare le medie che
% consentono di lisciare il segnale.
Navg = 100; nome = 'rcdisch_';
path = '/home/filippo/Desktop/CODICINI/LABO_PLASMI/Esperienze/Esperienza 0/Dati/Signals/electron_discharge/rcdisch/rcdisch';
car_ele = carica_segnale(path, nome, 33, 1e6, 0.001, 1, 100, 1);
...
function img_int = intensita_immagine(path, name, n)
    img_int = zeros(1, n);
    for i=1:n
        appo = [path '/' name num2str(i) '.tif'];
        M = imread(appo); M = double(M);
        img_int(i) = sum(sum(M));
    end
end

path = '/home/filippo/Desktop/CODICINI/LABO_PLASMI/Esperienze/Esperienza 0/Analisi/PlasmaPulite';
name = 'plasma_pulita';
n = 32;
