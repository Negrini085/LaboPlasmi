...
pathOut = '/home/filippo/Desktop/CODICINI/LABO_PLASMI/Esperienze/Esperienza 1/AnalisiRev/Frequenze/Risultati/Immagini/serie';
nu_r = 0; nu_rp = 0; nu_rpt = 0; off_pl = 0;
for i=1:9
    disp(['Analisi per serie ' num2str(i)])
    % Leggo Risultati
    path = [pathOff num2str(i) '.txt'];
    data = fopen(path, 'rt');
    N = 1; filescan = textscan(data,'%f %f %f','HeaderLines',N); offSerie = 45e-3/419 * filescan {1,3};

    pathr = [pathOut num2str(i) '_r.txt']; pathrp = [pathOut num2str(i) '_rp.txt']; pathrpt = [pathOut num2str(i) '_rpt.txt'];
    nuSerie_r = zeros(1, length(offSerie)); nuSerie_rp = zeros(1, length(offSerie)); nuSerie_rpt = zeros(1, length(offSerie));

    datar = fopen(pathr,'w'); fprintf(datar, '%s \n', '#        Nu1');
    datarp = fopen(pathrp,'w'); fprintf(datarp, '%s \n', '#        Nu1');
    datarpt = fopen(pathrpt,'w'); fprintf(datarpt, '%s \n', '#        Nu1');
    for j=1:length(offSerie)
        nuSerie_r(j) = nu_lin * 1/(1-(offSerie(j)/Rw)^2); fprintf(datar, '%s \n', [num2str(i) '     ' num2str(nuSerie_r(j))]);
        nuSerie_rp(j) = nu_lin * (1 + ((1-2*(Rp/Rw)^2)/(1-(Rp/Rw)^2)^2) * (offSerie(j)/Rw)^2); fprintf(datarp, '%s \n', [num2str(i) '     ' num2str(nuSerie_rp(j))]);
        nuSerie_rpt(j) =  nuSerie_rp(j) * (1 + (2.405/2 * (0.25 + log(Rw/Rp) + (4*pi*eps0*T*Lp)/(Qp) - 0.671)*(Rw/Lp))); fprintf(datarpt, '%s \n', [num2str(i) '     ' num2str(nuSerie_rpt(j))]);
    end

    fclose(datar); fclose(datarp); fclose(datarpt);
